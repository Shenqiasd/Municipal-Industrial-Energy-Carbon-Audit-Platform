...
- Sheet 3：A5 文案改为“单位主管节能领导姓名/职务”，A7 改为“能源管理负责人姓名”，A28 改为“3.节能降碳指标”；部分 F 列由合并空白改为固定 `-`，字段结构不变。为避免多行写入 `de_tech_indicator` 时撞历史唯一索引，迁移会将 `(enterprise_id,audit_year,indicator_year)` 改为普通索引，并新增 `row_seq` 记录模板行序号。
